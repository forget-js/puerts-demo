 ...
 * 运行时配置合并与导出.
 *
 * 以 config.default 为基线, 叠加当前构建 profile (config.active.ts); 对外 export Config 与别名 CC.
 */

import { configDefault, type AppConfig } from './Env/config.default';
import { activeConfigProfile, configActive } from './Env/config.active';
...
/** 合并后的运行时配置. */
export const Config: AppConfig = mergeConfig(configDefault, configActive);

/** 当前构建选中的配置 profile, 由 Scripts/select-config-profile.mjs 生成. */
export const ConfigProfile = activeConfigProfile;
